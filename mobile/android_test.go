...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gmpc.LightScryptN, Gmpc.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.modernizingpark", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-modernizingpark issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gmpc-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/modernizingpark/gethtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gmpc and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.modernizingpark", "github.com/go-modernizingpark/go-modernizingpark/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gmpc.aar"), "gmpc.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "modernizingpark", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.modernizingpark.gethtest"
