...
// SnapshotTree is an Modernizingpark state snapshot tree. It consists of one persistent
...
type Tree struct {
	diskdb mpcdb.KeyValueStore      // Persistent database to store the snapshot
...
// background thread.
func New(diskdb mpcdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool) *Tree {
