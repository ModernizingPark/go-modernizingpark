...
func InitDatabaseFromFreezer(db mpcdb.Database) {
...
func iterateTransactions(db mpcdb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db mpcdb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db mpcdb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
