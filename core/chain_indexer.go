...
type ChainIndexer struct {
	chainDb  mpcdb.Database      // Chain database to index the data from
	indexDb  mpcdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb mpcdb.Database, indexDb mpcdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
