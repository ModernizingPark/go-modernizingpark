...
type downloadTester struct {
	...
	stateDb mpcdb.Database // Database used by the tester for syncing from peers
	peerDb  mpcdb.Database // Database of the peers containing all data
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", modernizingpark.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", modernizingpark.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", modernizingpark.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", modernizingpark.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want modernizingpark.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", modernizingpark.SyncProgress{
