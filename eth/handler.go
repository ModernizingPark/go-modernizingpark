...
// NewProtocolManager returns a new Modernizingpark sub protocol manager. The Modernizingpark sub protocol manages peers capable
// with the Modernizingpark network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb mpcdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Modernizingpark peer", "peer", id)

	// Unregister the peer from the downloader and Modernizingpark peer set
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Modernizingpark protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Modernizingpark peer connected", "name", p.Name())

	// Execute the Modernizingpark handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkID, pm.forkFilter); err != nil {
		p.Log().Debug("Modernizingpark handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p, pm.removePeer); err != nil {
		p.Log().Error("Modernizingpark peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Modernizingpark message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Modernizingpark sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Modernizingpark network ID (128=mainnet 256=testnet)
