...
import (
	...
	lps "github.com/modernizingpark/go-modernizingpark/les/lespay/server"
...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb mpcdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light Modernizingpark peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), h.server); err != nil {
		p.Log().Debug("Light Modernizingpark handshake failed", "err", err)
		return err
	}
	if p.server {
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light Modernizingpark peer rejected", "err", errFullClientPool)
	...
	if err := h.server.peers.register(p); err != nil {
		...
		p.Log().Error("Light Modernizingpark peer registration failed", "err", err)
		return err
	}
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light modernizingpark response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light Modernizingpark message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Modernizingpark message arrived", "code", msg.Code, "bytes", msg.Size)
