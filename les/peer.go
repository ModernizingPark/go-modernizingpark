...
import (
	...
	"github.com/modernizingpark/go-modernizingpark/les/utils"
	...
	return p.handshake(td, head, headNum, genesis, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local modernizingpark node is running in archive mode, advertise ourselves we have
...
// participating in the Light Modernizingpark sub-protocol.
...
// for us(server) but for other protocols(e.g. MPC) upon the devp2p they
// may be useful.
