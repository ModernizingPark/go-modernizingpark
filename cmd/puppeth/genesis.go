...
import (
	...
	math2 "github.com/modernizingpark/go-modernizingpark/common/math"
...
// C++ Modernizingpark implementation.
...
// newAlethGenesisSpec converts a go-modernizingpark genesis block into a Aleth-specific
// chain specification format.
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only ethash is currently supported between go-modernizingpark and aleth
...
type parityChainSpec struct {
	...
	Genesis struct {
		Seal struct {
			Modernizingpark struct {
				Nonce   types.BlockNonce `json:"nonce"`
				MixHash hexutil.Bytes    `json:"mixHash"`
			} `json:"modernizingpark"`
		} `json:"seal"`
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-modernizingpark/pull/11039,
...
// newParityChainSpec converts a go-modernizingpark genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-modernizingpark and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// https://github.com/modernizingpark/EIPs/blob/master/EIPS/eip-608.md
	spec.Params.EIP150Transition = hexutil.Uint64(genesis.Config.EIP150Block.Uint64())

	// Spurious Dragon: 155, 160, 161, 170
	// https://github.com/modernizingpark/EIPs/blob/master/EIPS/eip-607.md
	...
	// gmpc has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.Modernizingpark.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.Modernizingpark.MixHash = (genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// Python Modernizingpark implementation.
...
// newPyEthereumGenesisSpec converts a go-modernizingpark genesis block into a Parity specific
// chain specification format.
func newPyEthereumGenesisSpec(network string, genesis *core.Genesis) (*pyEthereumGenesisSpec, error) {
	// Only ethash is currently supported between go-modernizingpark and pyethereum
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
