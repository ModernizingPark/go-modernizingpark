...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gmpc --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gmpc --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --mpcstats \'{{.Ethstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh
