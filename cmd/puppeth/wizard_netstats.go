...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for mpcstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["mpcstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["mpcstats"] = infos.Report()
		mpcstats = infos.config
	}
