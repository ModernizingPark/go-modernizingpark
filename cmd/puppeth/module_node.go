...
// nodeDockerfile is the Dockerfile required to run an Modernizingpark node.
var nodeDockerfile = `
FROM modernizingpark/client-go:latest
...
{{end}}
RUN \
  echo 'gmpc --cache 512 init /genesis.json' > gmpc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.modernizingpark/keystore/ && cp /signer.json /root/.modernizingpark/keystore/' >> gmpc.sh && \{{end}}
	echo $'exec gmpc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --mpcstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gmpc.sh

ENTRYPOINT ["/bin/sh", "gmpc.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Modernizingpark node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.modernizingpark{{if .Ethashdir}}
...
// deployNode deploys a new Modernizingpark node container to a remote machine via SSH,
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethstats":   config.mpcstats[:strings.Index(config.mpcstats, ":")],
		"Etherbase":  config.etherbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.mpcstats,
	}
	if info.gasTarget > 0 {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gmpc --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.modernizingpark"],
		ethashdir:  infos.volumes["/root/.ethash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		mpcstats:   infos.envvars["STATS_NAME"],
