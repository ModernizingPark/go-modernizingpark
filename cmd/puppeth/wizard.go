...
type config struct {
	...
	mpcstats  string   // Ethstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Modernizingpark services known to be running on servers
...
// it to an Modernizingpark address.
...
// converts it to an Modernizingpark address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by mpcstats, which compares IPs textually, not by value.
