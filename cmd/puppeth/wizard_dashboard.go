...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"mpcstats", "explorer", "wallet", "faucet"} {
	...
	// If we have mpcstats running, ask whether to make the secret public or not
	if w.conf.mpcstats != "" {
		fmt.Println()
		fmt.Println("Include mpcstats secret on dashboard (y/n)? (default = yes)")
