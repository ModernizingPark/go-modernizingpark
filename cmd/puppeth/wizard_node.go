...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.mpcstats == "" {
		log.Error("No mpcstats server configured")
		return
	}
	...
	if infos.mpcstats == "" {
		...
		infos.mpcstats = w.readString() + ":" + w.conf.mpcstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.mpcstats)
		infos.mpcstats = w.readDefaultString(infos.mpcstats) + ":" + w.conf.mpcstats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Modernizingpark node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
