...
func newClient(ctx *cli.Context) *mpcclient.Client {
	client, err := mpcclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, mpcclient.NewClient(client))
	if err != nil {
