...
func newClient(url string) *mpcclient.Client {
	client, err := mpcclient.Dial(url)
	if err != nil {
		utils.Fatalf("Failed to connect to Modernizingpark node: %v", err)
	}

	return client
}

func newClients(urls []string) []*mpcclient.Client {
	clients := make([]*mpcclient.Client, 0)
...
// newAccount creates a modernizingpark account with bind transactor by plaintext key string in hex format .
...
// newRandomAccount generates a random modernizingpark account with bind transactor
...
func sendEtherToRandomAccount(mainAccount *bind.TransactOpts, accounts []*bind.TransactOpts, amount *big.Int, token common.Address, client *mpcclient.Client) {
...
func generateSignedTransactions(total int, accounts []*bind.TransactOpts, amount *big.Int, token common.Address, client *mpcclient.Client) (txs []*types.Transaction) {
	// total txs
	workFn := func(start, end int, data ...interface{}) []interface{} {
...
func waitForTx(hash common.Hash, client *mpcclient.Client) {
...
func stressSendTransactions(txs []*types.Transaction, threads int, clients []*mpcclient.Client, client *mpcclient.Client) {
