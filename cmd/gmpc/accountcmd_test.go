...
func TestAccountListEmpty(t *testing.T) {
	gmpc := runGeth(t, "account", "list")
	gmpc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gmpc := runGeth(t, "account", "list", "--datadir", datadir)
	defer gmpc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gmpc := runGeth(t, "account", "new", "--lightkdf")
	defer gmpc.ExpectExit()
	gmpc.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gmpc := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gmpc.ExpectExit()
	gmpc.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gmpc := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gmpc.ExpectExit()
	gmpc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gmpc.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gmpc.StderrText(), m) {
...
// https://github.com/modernizingpark/go-modernizingpark/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gmpc.SetTemplateFunc("keypath", func(file string) string {
