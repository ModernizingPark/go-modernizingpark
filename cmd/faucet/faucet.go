// Copyright 2017 The go-modernizingpark Authors
...
import (
	...
	"github.com/modernizingpark/go-modernizingpark/cmd/utils"
	...
	"github.com/modernizingpark/go-modernizingpark/mpcclient"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Modernizingpark protocol")
	statsFlag   = flag.String("mpcstats", "", "Ethstats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Modernizingpark and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Modernizingpark address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Modernizingpark light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Modernizingpark protocol stack
	client *mpcclient.Client   // Client connection to the Modernizingpark chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Modernizingpark light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Modernizingpark service: %w", err)
	}

	// Assemble the mpcstats monitoring and reporting service'
	if stats != "" {
		if err := mpcstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := mpcclient.NewClient(api)
...
// close terminates the Modernizingpark connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Modernizingpark address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/go-modernizingpark/go-modernizingpark/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Modernizingpark address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// and scrape it for the Modernizingpark address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Modernizingpark address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Modernizingpark address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Modernizingpark address and profile URL.
	res, err := http.Get(url)
	if err != nil {
		return "", "", common.Address{}, err
	}
...
// authNoAuth tries to interpret a faucet request as a plain Modernizingpark address,
