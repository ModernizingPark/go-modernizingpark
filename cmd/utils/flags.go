// Copyright 2015 The go-modernizingpark Authors
...
// Package utils contains internal helper functions for go-modernizingpark commands.
package utils
...
import (
	...
	"github.com/modernizingpark/go-modernizingpark/accounts/keystore"
	"github.com/modernizingpark/go-modernizingpark/common"
	"github.com/modernizingpark/go-modernizingpark/common/fdlimit"
	...
	"github.com/modernizingpark/go-modernizingpark/eth"
	...
	"github.com/modernizingpark/go-modernizingpark/mpcstats"
	"github.com/modernizingpark/go-modernizingpark/graphql"
	"github.com/modernizingpark/go-modernizingpark/internal/ethapi"
	"github.com/modernizingpark/go-modernizingpark/internal/flags"
	"github.com/modernizingpark/go-modernizingpark/les"
	...
	"github.com/modernizingpark/go-modernizingpark/metrics/influxdb"
	...
	"github.com/modernizingpark/go-modernizingpark/p2p/nat"
	"github.com/modernizingpark/go-modernizingpark/p2p/netutil"
...
var (
	...
	EthStatsURLFlag = cli.StringFlag{
		Name:  "mpcstats",
		Usage: "Reporting URL of a mpcstats service (nodename:secret@host:port)",
	}
...
// for Gmpc and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gmpc account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "MPC", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func SetShhConfig(ctx *cli.Context, stack *node.Node) {
	if ctx.GlobalIsSet(WhisperEnabledFlag.Name) ||
		...
		log.Warn("Whisper support has been deprecated and the code has been moved to github.com/modernizingpark/whisper")
	}
}
...
// RegisterEthService adds an Modernizingpark client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) ethapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Modernizingpark service: %v", err)
		}
...
// RegisterEthStatsService configures the Modernizingpark Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := mpcstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Modernizingpark Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gmpc.", tagsMap)
		}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) mpcdb.Database {
	var (
		cache   = ctx.GlobalInt(CacheFlag.Name) * ctx.GlobalInt(CacheDatabaseFlag.Name) / 100
		handles = makeDatabaseHandles()

		err     error
		chainDb mpcdb.Database
	)
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb mpcdb.Database) {
	var err error
	chainDb = MakeChainDatabase(ctx, stack)
...
// new format.
//
// e.g. gmpc account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gmpc --keystore /tmp/mykeystore --lightkdf account new
//
